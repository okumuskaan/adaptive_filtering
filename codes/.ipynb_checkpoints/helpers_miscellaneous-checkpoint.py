...
def denoising_plots(t, e, s, fs, T, margin=100, conv_div=40, bufsize=100, type=None, save=False, savedfname="Example"):
    ...
    axes[0].legend()
    #axes[0].set_title("Source and Estimated Sound Signal", fontsize = 'xx-large')
    axes[0].set_xlabel("Time (sec)", fontsize = 'x-large')

    
    axes[1].plot(t, aee)
    axes[1].set_xlabel("Time (sec)", fontsize = 'x-large')
...
def compare_with_params(f, T, fs, mus=None, Ks=None, normalized=False, a=0.0, type="standard", margin=100, conv_div=40, bufsize=100, save=False, savedfname="Example"):
    ...
    if param_name=="K":
        ...
        ax.text(4, text_posy, "Not Converged Region!", color='black', ha='center', va='center', fontweight='heavy', fontsize='large', rotation=90)
    else:
        xlab = "Step Size, $\mu$"
    ...
    ax.set_ylabel("convergence points", color='tab:blue', fontsize = 'x-large')
    ax2.set_ylabel("max misadjustments", color='tab:red', fontsize = 'x-large')
