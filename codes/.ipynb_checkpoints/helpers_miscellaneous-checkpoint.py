...
def find_misadjustment(aee, conv_point, fs, margin=100, print_misad=True, find_mean=False):
    ...
    return misadjustment#np.max(temp)#, np.mean(temp)
...
def denoising_plots(t, e, s, fs, T, margin=100, conv_div=40, bufsize=100, type=None, save=False, savedfname="Example", find_mean_adj = False):
    ...
    if conv_point!=-1:
        misadjustment = 0
        if find_mean_adj:
            misadjustment = find_misadjustment(aee, conv_point, fs, margin, find_mean=True)
        else:
            misadjustment = find_misadjustment(aee, conv_point, fs, margin)
        ...
        axes[1].text(max_t, misadjustment+max_aee/30, "Misadjustment: " + f"{misadjustment:.2f}", color="red", ha='right', fontweight='bold', fontsize='large')
...
def denoising_plots_across_params(t, s, x, d, fs, T, normalized=False, a=0.0, rls_bool = False, defaultK =5, rls_mu = None, rls_eps = None,lms_type="standard", Ks=None, mus=None, margin=100, conv_div=40, bufsize=100, beta=None, save=False, savedfname="Example"):
    ...
    if Ks is not None:
        ...
        params = mus
        param_name = "mu"
    elif rls_mu is not None:
        print("rls_mu")
        params = rls_mu
        param_name = "rls_mu"
    elif rls_eps is not None:
        params = rls_eps
        param_name = "rls_eps"
    if params is None:
